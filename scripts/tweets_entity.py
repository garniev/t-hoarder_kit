...
class Counters(object):
    ...
    def write_top(self, top, type_top):
        f_file = codecs.open(self.path_experiment + self.prefix + '_' + type_top + '.txt', 'w', encoding='utf-8',
                             errors='ignore')
        for item in top:
            f_file.write('%s\n' % (item))
        f_file.close()
    ...
    def get_tweets_day(self):
        ...
        for (key, value) in self.tweets_day_order:
            ...
            key, self.dict_tweets_day.getitem(key), self.dict_RT_day.getitem(key), self.dict_reply_day.getitem(key),
            self.dict_mention_day.getitem(key)))
        f_out.close()
        return
    ...
    def get_authors_day(self):
        ...
        for (key, value) in self.tweets_day_order:
            f_out.write('%s\t%s\t%s\t%s\n' % (
            key, self.dict_tweets_day.getitem(key), self.dict_authors_unique_day.getitem(key),
            self.dict_authors_new_day.getitem(key)))
        f_out.close()
    ...
    def get_users_mention(self):
        ...
        top_size = self.top_size
        users_mention_rank_order = sorted([(value, key) for (key, value) in self.dict_users_mention.items()],
                                          reverse=True)
...
def main():
    ...
    # end loop first pass
    end_time = datetime.datetime(year=int(year), month=int(month), day=int(day), hour=int(hour), minute=int(minutes),
                                 second=int(seconds))
