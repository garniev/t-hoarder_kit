        ...
        try:
            ...
            for statuse in page:
                ...
                try:
                    ...
                    statuse.id, statuse.created_at, statuse.author.screen_name, text, statuse.source, statuse.user.id,
                    statuse.user.followers_count, statuse.user.friends_count, statuse.user.statuses_count, location,
                    url_expanded, url_media, geoloc, statuse.retweet_count, statuse.retweeted,
                    statuse.in_reply_to_user_id)
                    # print tweet
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='This script extracts the content of a set of tweets for grouping them in days, additionally it takes note of authors, hashtag, words and specific sentences in each day.')
    ...
    f_out.write(
        'User\tRole\th-index\tFollowers/Following\tlog_followers\tRatio RT_in\tRatio_RT_out\tRatio Reply_out\tFrequency\tRatio multimedia\tRatio URL\tRatio HT\n')
    for line in f_users_group_file:
        ...
        user, role, h_index, ratio_ff, log_followers, ratio_RT_in, ratio_RT_out, ratio_reply_out, frequency,
        ratio_multimedia, ratio_url, ratio_ht))
        del whatismyrole
    f_out.close()
    f_log.close()
