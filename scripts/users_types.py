...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='ls this script classifies users into categories according to their activity')
    ...
    f_out.write(
        'user\trole\tinf_ff\tlog_ff\trts_in\trts_out\trts_in_uniq\trts_out_uniq\treplies_in\treplies_out\treplies_in_uniq\treplies_out_uniq\tmentions_in\tmentions_out\tmentions_in_uniq\tmentions_out_uniq\n')
    for line in f_in:
        if head:
            head = False
        else:
            ...
            if tweet_flat == None:
                ...
                if num_tweets % 10000 == 0:
                    print num_tweets
                num_tweets += 1
                (id_tweet, year, month, day, hour, minutes, seconds, author, text, app, user_id, followers,
                 following) = tweet_flat
                author = author.lower()
                text = text.lower()
